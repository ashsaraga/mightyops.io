...
<article id="progress_<?php echo the_title(); ?>" <?php post_class( $homeClasses ); ?>>
	<div>
		<h4 class="is-size-5 has-margin-none has-text-weight-regular has-letter-spacing-2">
			<?php echo the_title(); ?>
			<span class="has-text-<?php echo $iconSet; ?> has-margin-none has-padding-8 has-padding-left is-size-4 has-text-weight-regular <?php if ( $stack == 'front-end' ) { echo 'has-text-mod3'; } else if ( $stack == 'back-end' ) { echo 'has-text-mod5'; } ?>"><?php echo $icon; ?></span>
		</h4>
